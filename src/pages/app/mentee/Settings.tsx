...
import { Settings, Globe, Calendar, Clock, MapPin, Check, ChevronDown, X, Loader2, Filter, Power, Link2, RefreshCw, AlertCircle, ChevronLeft, ChevronRight, Search } from "lucide-react";
...
const ringIndigo = "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-indigo-500/40";

/** ──────────────────────────────────────────────────────────────────────────
 *  LIGHT Helpers
 *  ────────────────────────────────────────────────────────────────────────── */
const Card: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ className = "", ...props }) => <div className={`rounded-2xl ${surface} ${border} border ${className}`} {...props} />;
...
const FieldRow: React.FC<{ children: React.ReactNode }> = ({ children }) => <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{children}</div>;

const Label: React.FC<{ children: React.ReactNode; htmlFor?: string }> = ({ children, htmlFor }) => (
    <label htmlFor={htmlFor} className={`text-xs font-medium ${subtext} select-none`}>
        {children}
    </label>
);

const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { right?: React.ReactNode }> = ({ right, className = "", ...rest }) => (
    <div className={`h-11 ${surfaceSolid} ${border} border rounded-xl flex items-center px-3 ${ringIndigo}`}>
        <input className={`flex-1 bg-transparent outline-none text-sm ${text} placeholder-zinc-500 ${className}`} {...rest} />
        {right}
    </div>
);

const TextArea: React.FC<React.TextareaHTMLAttributes<HTMLTextAreaElement>> = ({ className = "", ...rest }) => <textarea className={`min-h-[84px] ${surfaceSolid} ${border} border rounded-xl px-3 py-2 text-sm ${text} placeholder-zinc-500 ${ringIndigo} w-full ${className}`} {...rest} />;

const Switch: React.FC<{ checked: boolean; onChange: (v: boolean) => void }> = ({ checked, onChange }) => (
    <button type="button" onClick={() => onChange(!checked)} className={`h-6 w-11 rounded-full transition relative ${checked ? "bg-indigo-600" : "bg-zinc-300 dark:bg-zinc-700"} ${ringIndigo}`} aria-pressed={checked}>
        <span className={`absolute top-[3px] left-[3px] h-[18px] w-[18px] rounded-full bg-white dark:bg-zinc-100 transition-transform ${checked ? "translate-x-5" : "translate-x-0"}`} />
    </button>
);
...
> = ({ variant = "solid", className = "", children, ...rest }) => {
    const map = {
        solid: "text-white bg-gradient-to-r from-indigo-600 via-indigo-500 to-violet-500 shadow-sm hover:brightness-[.98]",
        ghost: "border border-zinc-300 dark:border-zinc-700 text-zinc-900 dark:text-zinc-100 bg-white/60 dark:bg-zinc-900/40 hover:bg-white/80 dark:hover:bg-zinc-900/60",
    ...
    return (
        <button className={`inline-flex items-center gap-2 rounded-xl px-3.5 py-2 text-sm font-semibold transition ${ringIndigo} ${map[variant]} ${className}`} {...rest}>
            {children}
        </button>
    );
};
...
const SelectMenu: React.FC<{
    ...
    return (
        <div className={`relative ${className}`} ref={ref}>
            <button type="button" className={`h-11 w-full ${surfaceSolid} ${border} border rounded-xl px-3 text-left text-sm ${text} flex items-center justify-between ${ringIndigo}`} onClick={() => setOpen((v) => !v)} aria-haspopup="menu" aria-expanded={open}>
            ...
            <div className={`absolute z-30 mt-2 w-full ${surfaceElev} ${border} border rounded-xl p-1 shadow-lg transition ${open ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-1 pointer-events-none"}`} role="menu">
                {options.map((o) => (
                    <button
                        key={o.value}
                        className={`w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-50/90 dark:hover:bg-zinc-800/70 text-sm flex items-center gap-2 ${o.value === value ? "font-semibold" : ""}`}
                        onClick={() => {
                            onChange(o.value);
                            setOpen(false);
                        }}
...
const Paginator: React.FC<{
    ...
    <div className="flex items-center justify-between mt-4">
        ...
        <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" onClick={onNext} disabled={page >= totalPages || totalPages === 0}>
...
const ME_SettingsGeneral: React.FC = () => {
    ...
    const appsFiltered = APPS.filter((a) => {
        ...
        if (q && !`${a.name} ${a.type} ${a.status}`.toLowerCase().includes(q)) return false;
        return true;
    });
    ...
    const addrFiltered = ADDRS.filter((a) => {
        ...
        if (q && !`${a.label} ${a.line1} ${a.city} ${a.country}`.toLowerCase().includes(q)) return false;
        return true;
    });
            ...
            <div className="px-4 sm:px-6 lg:px-8 py-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <div>
                        ...
                        <p className={`text-sm ${subtext}`}>Configure language, time, and default booking behaviour.</p>
                    </div>
                    <div className="flex items-center gap-2">
                ...
                <div className="mt-4 overflow-x-auto">
                    <div className={`inline-flex items-center gap-2 ${surface} ${border} border rounded-xl p-1`}>
                        {[
                            { id: "prefs", label: "Preferences" },
                            { id: "apps", label: "Connected Apps" },
                            { id: "addresses", label: "Addresses" },
                        ].map((t) => (
                            <button key={t.id} onClick={() => setTab(t.id as any)} className={`px-3 py-1.5 rounded-lg text-sm transition ${tab === t.id ? "bg-indigo-600 text-white" : "hover:bg-zinc-100 dark:hover:bg-zinc-800"}`}>
                                {t.label}
                            </button>
                        ))}
                    </div>
                ...
                <div className="mt-6 space-y-6">
                    {tab === "prefs" && (
                        <Card className="p-4">
                            <SectionTitle title="Regional & Booking" desc="Your default locale, timezone and how bookings behave." />
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div className="flex items-center justify-between rounded-xl px-3 py-2.5 border border-zinc-200/60 dark:border-zinc-800">
                                        <div>
                                            ...
                                            <div className={`text-xs ${subtext}`}>Limit large animations for accessibility.</div>
                                        </div>
                                        <Switch checked={a11yReduceMotion} onChange={setReduceMotion} />
                                    </div>

                                    <div className="flex items-center justify-between rounded-xl px-3 py-2.5 border border-zinc-200/60 dark:border-zinc-800">
                                        <div>
                                            <div className={`text-sm ${text}`}>Auto add to calendar</div>
                                            <div className={`text-xs ${subtext}`}>New bookings will be added to your calendar.</div>
                                        </div>
                    ...
                    {tab === "apps" && (
                        <Card className="p-4">
                            <SectionTitle title="Connected Apps" desc="Manage integrations like calendars, conferencing, and dev tools." />

                            {/* Filter bar (responsive, non-pushing) */}
                            <div className="mt-4 grid grid-cols-1 md:grid-cols-12 gap-2">
                                <div className="md:col-span-5">
                                    <div className={`h-11 ${surfaceSolid} ${border} border rounded-xl px-3 flex items-center gap-2 ${ringIndigo}`}>
                            ...
                            <div className="mt-4 grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
                                {appsRows.map((a) => (
                                    <div key={a.id} className={`rounded-xl ${surfaceSolid} ${border} border p-3`}>
                                        <div className="flex items-start justify-between gap-2">
                                            <div>
                                                <div className={`text-sm font-semibold ${text}`}>{a.name}</div>
                                                <div className={`text-xs ${subtext}`}>
                                                    {a.type} • last sync {rel(a.lastSync)}
                                                </div>
                                            </div>
                                            <span className={`px-2 py-1 rounded-lg border text-[11px] ${a.status === "connected" ? "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300 dark:border-emerald-900" : a.status === "error" ? "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/40 dark:text-amber-300 dark:border-amber-900" : "bg-zinc-100 text-zinc-700 border-zinc-200 dark:bg-zinc-800/50 dark:text-zinc-300 dark:border-zinc-700"}`}>{a.status}</span>
                                        </div>
                            ...
                            {/* Pagination */}
                            <Paginator page={pApp} totalPages={appsPages} onPrev={() => setPApp((p) => Math.max(1, p - 1))} onNext={() => setPApp((p) => Math.min(appsPages, p + 1))} />
                        </Card>
                    )}

                    {tab === "addresses" && (
                        <Card className="p-4">
                            <SectionTitle title="Addresses" desc="Used for billing and receipts." />
                            ...
                            <div className="hidden md:block mt-4 overflow-x-auto">
                                <table className="w-full text-sm">
                                    ...
                                    <tbody className="divide-y divide-zinc-200/60 dark:divide-zinc-800">
                                        {addrRows.map((a) => (
                                            <tr key={a.id}>
                                                <td className="px-3 py-2 font-semibold">
                                                    {a.label} {a.default && <span className="ml-2 text-[11px] px-2 py-0.5 rounded-lg border border-emerald-200 dark:border-emerald-900 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300">default</span>}
                                                </td>
                                        ...
                                        {addrRows.length === 0 && (
                                            <tr>
                                                <td className={`px-3 py-4 text-center ${subtext}`} colSpan={5}>
                            ...
                            <div className="grid md:hidden grid-cols-1 gap-3 mt-4">
                                ...
                                {addrRows.length === 0 && <div className={`text-sm text-center ${subtext}`}>No addresses match your filters.</div>}
                            </div>

                            <Paginator page={pAddr} totalPages={addrPages} onPrev={() => setPAddr((p) => Math.max(1, p - 1))} onNext={() => setPAddr((p) => Math.min(addrPages, p + 1))} />
                        </Card>
                    )}
                </div>
            </div>
