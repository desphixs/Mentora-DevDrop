...
const btn = "inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold transition " + ringIndigo;
        ...
        <div className="min-h-screen bg-[radial-gradient(1000px_500px_at_10%_-10%,rgba(79,70,229,0.12),transparent),radial-gradient(900px_500px_at_90%_-10%,rgba(99,102,241,0.12),transparent)]">
            ...
            <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
                ...
                <div className="text-center">
                    <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight leading-tight text-[#0F132E] dark:text-[#E9ECFF]">Let’s talk</h1>
                    <p className="mt-3 max-w-2xl mx-auto text-sm sm:text-base text-[#5E66A6] dark:text-[#A7B0FF]/85">Questions about pricing, integrations, or getting started? We’ll get back within one business day.</p>
                </div>
                ...
                <div className="mt-10 grid lg:grid-cols-3 gap-6">
                    ...
                    <div className={`lg:col-span-2 ${card} p-6`}>
                        ...
                        <form onSubmit={onSubmit} className="grid gap-3">
                            <div className="grid sm:grid-cols-2 gap-3">
                                <Field label="Your name" error={errors.name} input={<input value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} className={`h-11 w-full rounded-xl border border-[#E7E9FF] dark:border-[#2B2F55] bg-white/70 dark:bg-white/[0.06] px-3 text-sm ${ringIndigo}`} placeholder="Ada Lovelace" />} />
                                <Field label="Email" error={errors.email} input={<input type="email" value={form.email} onChange={(e) => setForm((p) => ({ ...p, email: e.target.value }))} className={`h-11 w-full rounded-xl border border-[#E7E9FF] dark:border-[#2B2F55] bg-white/70 dark:bg-white/[0.06] px-3 text-sm ${ringIndigo}`} placeholder="you@company.com" />} />
                            </div>

                            <Field
                                label="Topic"
                                input={
                                    <select value={form.topic} onChange={(e) => setForm((p) => ({ ...p, topic: e.target.value as Form["topic"] }))} className={`h-11 w-full rounded-xl border border-[#E7E9FF] dark:border-[#2B2F55] bg-white/70 dark:bg-white/[0.06] px-3 text-sm ${ringIndigo}`}>
                            ...
                            <Field label="Message" error={errors.message} input={<textarea rows={5} value={form.message} onChange={(e) => setForm((p) => ({ ...p, message: e.target.value }))} className={`w-full rounded-xl border border-[#E7E9FF] dark:border-[#2B2F55] bg-white/70 dark:bg-white/[0.06] px-3 py-2 text-sm ${ringIndigo}`} placeholder="How can we help?" />} />
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-3">
                                ...
                                <button type="submit" disabled={state === "sending"} className={`${btn} ${btnSolid} disabled:opacity-60`}>
                                    {state === "sending" ? (
                                        "Sending…"
                                    ) : (
                                        <>
                    ...
                    <div className="grid gap-3">
                        ...
                        <div className={`${card} p-4`}>
                            ...
                            <div className="mt-1 text-sm text-[#2C3157] dark:text-[#C9D1FF]/85">123 Market Street, Lagos, NG</div>
                        </div>
